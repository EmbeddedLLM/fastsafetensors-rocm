...
static void load_nvidia_functions() {
    ...
    if (handle_cudart) {
        ...
        if (cuda_found) {
            ...
            mydlsym(&cuda_fns.cudaDeviceMalloc, handle_cudart, "hipMalloc");
            mydlsym(&cuda_fns.cudaDeviceFree, handle_cudart, "hipFree");
            bool success = cuda_fns.hipMemcpy && cuda_fns.hipDeviceSynchronize && cuda_fns.hipHostAlloc && cuda_fns.hipHostFree && cuda_fns.hipDeviceGetPCIBusId && cuda_fns.cudaDeviceMalloc && cuda_fns.cudaDeviceFree;
            if (!success) {
                cuda_found = false;
                if (init_log) {
...
uintptr_t gpu_malloc(uint64_t length) {
    void *p;
    if (cuda_fns.cudaDeviceMalloc(&p, length) != hipSuccess) {
        return 0;
    }
    return reinterpret_cast<uintptr_t>(p);
}
