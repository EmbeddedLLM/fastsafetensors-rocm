...
static hipError_t cpu_cudaMemcpy(void * dst, const void * src, size_t size, enum hipMemcpyKind) {
    std::memcpy(dst, src, size);
    return hipSuccess;
}
static hipError_t cpu_cudaDeviceSynchronize() { return hipSuccess; }
static hipError_t cpu_cudaHostAlloc(void ** p, size_t length, unsigned int) {
    if (posix_memalign(p, ALIGN, length) < 0) {
        return hipErrorOutOfMemory;
    }
    return hipSuccess;
}
static hipError_t cpu_cudaFreeHost(void * p) {
    free(p);
    return hipSuccess;
}
static hipError_t cpu_cudaDeviceGetPCIBusId(char * in, int s, int) {
    if (s > 0)
        in[0] = 0;
    return hipSuccess;
}
...
ext_funcs_t cpu_fns = ext_funcs_t {
    ...
    .cuFileRead = nullptr,
    .hipMemcpy = cpu_cudaMemcpy,
    .hipDeviceSynchronize = cpu_cudaDeviceSynchronize,
    .hipHostAlloc = cpu_cudaHostAlloc,
    .hipHostFree = cpu_cudaFreeHost,
    .hipDeviceGetPCIBusId = cpu_cudaDeviceGetPCIBusId,
...
static void load_nvidia_functions() {
    ...
    const char* cudartLib = "libamdhip64.so";
    ...
    if (handle_cudart) {
        mydlsym(&hipGetDeviceCount, handle_cudart, "hipGetDeviceCount");
        if (hipGetDeviceCount) {
            int count;
            if (hipGetDeviceCount(&count) != hipSuccess) {
                count = 0; // why hipGetDeviceCount returns non-zero for errors?
            }
            cuda_found = count > 0;
            if (init_log) {
            ...
            cuda_found = false;
            if (init_log) {
                fprintf(stderr, "[DEBUG] No hipGetDeviceCount, fallback!\n");
            }
        }
        if (cuda_found) {
            mydlsym(&cuda_fns.hipMemcpy, handle_cudart, "hipMemcpy");
            mydlsym(&cuda_fns.hipDeviceSynchronize, handle_cudart, "hipDeviceSynchronize");
            mydlsym(&cuda_fns.hipHostAlloc, handle_cudart, "hipHostAlloc");
            mydlsym(&cuda_fns.hipHostFree, handle_cudart, "hipHostFree");
            mydlsym(&cuda_fns.hipDeviceGetPCIBusId, handle_cudart, "hipDeviceGetPCIBusId");
            mydlsym(&cuda_fns.hipDeviceMalloc, handle_cudart, "hipMalloc");
            mydlsym(&cuda_fns.hipDeviceFree, handle_cudart, "hipFree");
            bool success = cuda_fns.hipMemcpy && cuda_fns.hipDeviceSynchronize && cuda_fns.hipHostAlloc && cuda_fns.hipHostFree && cuda_fns.hipDeviceGetPCIBusId && cuda_fns.hipDeviceMalloc && cuda_fns.hipDeviceFree;
            if (!success) {
                cuda_found = false;
                if (init_log) {
    ...
    if (!cuda_found) {
        cuda_fns.hipMemcpy = cpu_cudaMemcpy;
        cuda_fns.hipDeviceSynchronize = cpu_cudaDeviceSynchronize;
        cuda_fns.hipHostAlloc = cpu_cudaHostAlloc;
        cuda_fns.hipHostFree = cpu_cudaFreeHost;
        cuda_fns.hipDeviceGetPCIBusId = cpu_cudaDeviceGetPCIBusId;
    }
...
std::string get_device_pci_bus(int deviceId) {
    ...
    if (cuda_fns.hipDeviceGetPCIBusId) {
        err = cuda_fns.hipDeviceGetPCIBusId(pciBusId, 32, deviceId);
        if (err != hipSuccess) {
            std::fprintf(stderr, "get_device_pci_bus: hipDeviceGetPCIBusId failed, deviceId=%d, err=%d\n", deviceId, err);
            return "";
        }
    } else {
        return "";
...
uintptr_t gpu_malloc(uint64_t length) {
    void *p;
    if (cuda_fns.hipDeviceMalloc(&p, length) != hipSuccess) {
        return 0;
    }
    return reinterpret_cast<uintptr_t>(p);
}
...
void gpu_free(uintptr_t addr) {
    cuda_fns.hipDeviceFree(reinterpret_cast<void*>(addr));
}
...
const int gds_device_buffer::memmove(uint64_t _dst_off, uint64_t _src_off, const gds_device_buffer& _tmp, uint64_t length) {
    ...
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    err = _fns->hipMemcpy(tmp, src, length, hipMemcpyDefault);
    if (err != hipSuccess) {
        std::printf("gds_device_buffer.memmove: hipMemcpy[0](tmp=%p, src=%p, length=%" PRIu64 ") failed, err=%d\n", tmp, src, length, err);
        return -1;
    }
    err = _fns->hipMemcpy(dst, tmp, length, hipMemcpyDefault);
    if (err != hipSuccess) {
        std::printf("gds_device_buffer.memmove: hipMemcpy[1](dst=%p, tmp=%p, length=%" PRIu64 ") failed, err=%d\n", dst, tmp, length, err);
        return -1;
    }
    if (debug_log) {
        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
void nogds_file_reader::_thread(const int thread_id, ext_funcs_t *fns, const int fd, const gds_device_buffer& dst, const int64_t offset, const int64_t length, const uint64_t ptr_off, thread_states_t *s) {
    ...
    while (count < length) {
        ...
        err = fns->hipMemcpy(dst._get_raw_pointer(ptr_off + count, c), buffer, c, hipMemcpyHostToDevice);
        if (err != hipSuccess) {
            std::printf("nogds_file_reader._thread: hipMemcpy(%p, %p, %" PRIi64 ") failed, err=%d\n", dst._get_raw_pointer(ptr_off + count, c), buffer, count, err);
            failed = true;
            goto out;
        } else if (c <= 64 * 1024) {
            fns->hipDeviceSynchronize();
        }
        count += c;
        if (debug_log) {
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
const int nogds_file_reader::submit_read(const int fd, const gds_device_buffer& dst, const int64_t offset, const int64_t length, const uint64_t ptr_off)
{
    ...
    if (this->_s._read_buffer == nullptr) {
        ...
        err = _fns->hipHostAlloc(&this->_s._read_buffer, buf_len, 0);
        if (err != hipSuccess) {
            std::printf("nogds_file_reader.submit_read: hipHostAlloc(%" PRIi64 ") failed\n", buf_len);
            return -1;
        }
        mc.bounce_buffer_bytes += buf_len;
        if (debug_log) {
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
            std::printf("[DEBUG] nogds_file_reader.submit_read: hipHostAlloc, addr=%p, size=%" PRIi64 ", elapsed=%" PRId64 " us\n",
...
nogds_file_reader::~nogds_file_reader() {
    ...
    if (this->_s._read_buffer != nullptr) {
        auto buf_len = this->_s._bbuf_size_kb * 1024 * this->_s._max_threads;
        _fns->hipHostFree(this->_s._read_buffer);
        if (debug_log) {
            std::printf("[DEBUG] hipHostFree, addr=%p, size=%" PRIi64 "\n",
